...
export default {
  name: 'ConcordancesKeywordKWICList',
  components: {
  },
  ...
  methods: {
    ...
    clickOnLemma (item) {
      ...
      this.getConcordances({
        ...
        this.error = this.error_message_for(error,"keyword.concordances.",{400:"invalid_input",404:"not_found"});
      }).then(()=>{
      });
    }
  },
  created () {
    ...
    if(!this.keyword) return this.$router.push('/keyword/id'); // retry

    this.getCorpus(this.keyword.corpus).catch((error)=>{
      this.error = "Keyword Analysis or Corpus not Found: "+error.message;
    });
